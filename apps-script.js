...
/**
 * File: apps-script.js (Backend / Server-side)
 * Menerima pesanan dan mengonfirmasi pembayaran ke Google Sheets.
 */
...
function createOrder(cartData, customerInfo) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow(["Order ID", "Tanggal", "Nama", "Meja/Catatan", "Detail Pesanan", "Total", "Status"]);
      // Opsional: Format header menjadi bold
    ...
    cartData.forEach(function(item) {
      ...
      orderDetails.push(item.name + " (x" + item.quantity + ")");
    });
...
function confirmPayment(orderId) {
  try {
    ...
    // Cari baris berdasarkan Order ID (Asumsi Order ID ada di kolom A / index 0)
    ...
    if (rowToUpdate !== -1) {
      // Update kolom Status (Asumsi status ada di kolom G / index 7)
      sheet.getRange(rowToUpdate, 7).setValue("PAID");
      return { status: 'success' };
    } else {
      return { status: 'error', message: "Order ID tidak ditemukan di database." };
    }
